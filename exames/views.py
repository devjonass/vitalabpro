...
from .models import AcessoMedico, TiposExames, PedidoExames, SolicitacaoExame
    ...
    if not exame.requer_senha:
        if not exame.resultado:
            messages.add_message(
                request, constants.ERROR, "Ainda não foi cadastrado seu resultado, por favor entre em contato!")
...
@login_required
def solicitar_senha_exame(request):
    if request.method == "GET":
        tipos_exames = TiposExames.objects.all()
        return render(request, "solicitar_exames.html", {"tipos_exames": tipos_exames})
...
def cancelar_pedido(request, pedido_id):
    pedido = PedidoExames.objects.get(id=pedido_id)
...
def acesso_medico(request, token):
    ...
    pedidos = PedidoExames.objects.filter(data__gte=acesso_medico.data_exames_iniciais).filter(
